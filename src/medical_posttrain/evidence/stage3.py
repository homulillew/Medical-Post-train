"""Stage 3 durable run records and physical-attempt lineage."""
import json
import os
...
def prepare(purpose, run_class, config):
    run_id = 's3_' + purpose + '_' + time.strftime('%Y%m%dT%H%M%S', time.gmtime()) + '_' + uuid.uuid4().hex[:6]
    ...
    manifest = dict(run_id=run_id, stage=3, purpose=purpose, run_class=run_class,
...
def launch(out, action):
    ...
    attempt.mkdir()
    argv = [sys.executable, str(ROOT/'scripts/run_stage3.py'), 'worker', '--run', str(out), '--attempt', str(attempt)]
...
class Run:
    ...
    def finish(self, summary):
        summary = dict(summary, run_id=self.run_id)
        ...
        write_json(self.out/'status.json', dict(status=summary['status'], ended=now()))
