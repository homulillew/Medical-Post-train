"""Auxiliary native verl FSDP2/GRPO-advantage/token-level vanilla actor with explicit old-policy barrier.
...
from medical_posttrain.rl.common import read, immutable, event, record, durable
...
class Actor:
    def __init__(self, cfg, directory, adapter, checkpoint=None):
        ...
        self.loss_config = ActorConfig(strategy='fsdp2',rollout_n=4,ppo_micro_batch_size_per_gpu=1,
            ...
            ppo_mini_batch_size=cfg['mini_prompts'],ppo_epochs=1, loss_agg_mode='seq-mean-token-mean',
            clip_ratio=cfg['clip_ratio_low'],clip_ratio_c=3.0)
    ...
    def update(self, groups, directory, fixed_old=None, check_parity=False, step_before=0):
        ...
        for start in range(0,32,self.cfg['mini_prompts']*4):
            ...
            for i in range(start,end):
                ...
                loss,m = compute_policy_loss_vanilla(old[i:i+1],current,advantages[i:i+1],mask[i:i+1],config=self.loss_config,loss_agg_mode='seq-mean-token-mean')
            ...
            if self.cfg.get('fault_injection') and len(minibatches)==1:
                from medical_posttrain.rl.transactions import fault_point
        ...
        summary = dict(old_logprob_seconds=old_seconds,minibatches=minibatches,
            parameter_delta_l2=delta,initial_parameter_l2=sum(v.double().square().sum().item() for v in initial.values())**.5,
            policy_objective='verl.compute_policy_loss_vanilla',ratio_field_semantics='derived sequence ratio; native optimization uses token ratios',
            initial_trainable_digest=self.digest(initial),
