...
Final state-schema audit: an interim progress_observed_at annotation was outside the existing closed project-state schema. Removed it from the index; all contemporaneous timestamps remain in immutable raw progress/logs. Added schema validation to the Stage2 isolation gate. The earlier raw-evidence PASS receipt is retained; final verification is rerun with this additional check.
