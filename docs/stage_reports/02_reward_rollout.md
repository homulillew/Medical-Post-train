...
证据入口：[selected runs](../../experiments/stage2/selected_runs.json)、[正式摘要](../../experiments/stage2/s2_formal_20260909T025736_f607d9/summary.json)、[group明细](../../experiments/stage2/s2_formal_20260909T025736_f607d9/groups.json)、[奖励manifest](../../experiments/stage2/reward_manifest.json)、[决策记录](../implementation/STAGE2_DECISIONS.md)。完整响应、token IDs和embedding留在 `/data/WSH/medical-post-train-artifacts/runs/s2_formal_20260909T025736_f607d9`，Git只保存摘要/索引/案例和SHA。
...
处理记录：heldout cluster关联2617条、纯标点题2条、非连续选项标签11条、非法答案3条、train内部重复簇成员2821条。剩余49,043个清洁唯一候选，按seed42 SHA顺序选15k，每簇一个代表，没有复制补数。正式1000题使用独立`42:profiling:ID`哈希排序；smoke采用随后50题，互斥，不按生成表现选题。完整rejections及来源hash在data run `s2_data_20260909T024052_870aba`。

pool答案集合基数分布：{'1': 14504, '3': 175, '2': 117, '4': 132, '5': 72}。共有 **2220/15000** 条解释缺失，照实保留，不用模型补写；对应sem=0并记录missing_reference。用有解释样本筛选pool会改变目标分布，因此本轮没有这么做。

后续完整case复核另发现非空占位参考“请等待更新”：pool共4题，formal共1题/4条响应。冻结pipeline按非空文本编码了它们；对应低sem不能解释为医学推理差。实际分数不回填或改写，单独保留 [placeholder audit](../../experiments/stage2/reference_placeholder_audit.json)。未来若屏蔽此类文本，必须更新两个基线共用的reward版本。
...
源完整性限制：pool有34题命中预先描述性正则“暂无图/无图/如图/见图/下图/图示”，formal中3题。实际输入全为文本，没有图像附件。train27961明确“暂无图”，参考却描述影像特征，模型有时也讨论假设影像。这些题不事后删除，也不把得分解释为视觉诊断能力。该正则只是case triage，不是伪difficulty/category。

## 2. 固定policy与实际vLLM运行时
...
Formal policy_version：`cdb99bc34f05c71a7ec6411c19e7091562e8f8b3277961ceb83d33e8393a9ff1`。formal从clean git `3390cbafe55de6fde91fec6630c1248967772bb9`启动，固定config/source archive及hash，后续只增加分析/案例/报告。正式每批检查核心parser/reward/semantic/prompt/rollout源文件与manifest，开始/结束及批间重hashadapter。没有任何optimizer、GSPO、GRPO训练或refill。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| abhinand/MedEmbed-small-v0.1 | 0.9530 | 0.9377 | 0.4797 | 0.2584 | 2.480 | 1.093 |
| BAAI/bge-m3 | 0.9615 | 0.8481 | 0.6341 | 0.3371 | 13.093 | 3.023 |
...
正式reward_version（manifest SHA）：`cc72be1c2348d20f58bf978d047b9b4a2f200d8c70c39f0d0d98506528bfb905`。
## 5. Smoke与完整formal执行

Smoke `s2_smoke_20260909T024936_746dbe`：50题×4=200全部完成，accuracy57%，0/1/2/3/4为10/5/10/11/14，mixed52%。输出mean236.26、P95=353.30、max453，answer-tag closure100%、truncation0%、strict86.5%。48组四条文本都不同、2组三种文本，无极端采样collapse。因未触发truncation>5%或closure<95%，没有比较1536/2048；没有为了制造mixed而提高温度。

Formal `s2_formal_20260909T025736_f607d9`：预冻结1000个唯一prompt全部覆盖，各恰好4个有效唯一trajectory，同policy/config/reward。成功stop或length可计完成；engine异常、缺响应和重复ID不可计。完整raw group以临时文件+fsync+rename提交，之后才做reward，token IDs和原文不被过滤改写。每批request-start/completed WAL记录请求ID、seed、输入长度、输出token和时间。相同run恢复只消费尚未完成ID，保持配置；本轮实际失败/重试见成本表，不把未发生的恢复称为实测成功。
...
trajectory accuracy=51.775%，本次四次生成any-correct=76.50%。acc_vector逐组公开，无伪造advantage。std使用population ddof0，分别列correctness与hybrid_reward的std；尚未进行policy loss归一化，所以不报告actual advantage std。

P_mixed=0.531000，固定SFT policy下的一阶 expected sampling amplification≈**1.8832**。这不是已经运行Dynamic得到的最终倍率；后续policy、长度和acceptance都会变，prompt encounters也可能相关。当前mixed未低于10%的预设资源风险示例阈值；后续仍需实际refill和训练窗口测量。

Mixed归因补充：{'has_parsed_wrong_answers': 378, 'contrast_only_from_unparseable_responses': 153}。其中“contrast_only_from_unparseable_responses”意味着可解析响应全部答对，其他成员因格式无法解析而acc0。这样的group满足本项目acc对比定义，但不能全都解释为纯医学知识frontier。完整类别明细在 [supplementary analysis](../../experiments/stage2/supplementary_analysis.json)。

sampling_metric固定 **acc**。即使all-wrong，format可能不同，使total_reward std非零：正式观察到125组此情况；所有错误响应的raw sem虽可变化，门控后的语义贡献恒0。若按total std过滤，会误收这类缺correctness contrast的组。反过来，all-correct中有197组hybrid std非零，来自语义/格式变化；不能声称它们一律没有任何训练信号。Dynamic按合同删除它们，舍弃这部分可能的shaping排序信号以研究正确性对比干预；效果留给Stage4。
...
| token范围 | mean | P50 | P90 | P95 | P99 | max |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| reasoning_tokens | 226.93 | 240.00 | 325.00 | 354.05 | 421.01 | 766.00 |
| answer_tokens | 0.89 | 1.00 | 1.00 | 1.00 | 2.00 | 4.00 |
| output_tokens | 237.55 | 251.00 | 336.00 | 365.00 | 431.01 | 777.00 |

| group类型 | output mean | output P95 | reasoning mean |
| --- | ---: | ---: | ---: |
| all_wrong | 237.9691489361702 | 378.0 | 227.46063829787235 |
| mixed | 231.35546139359698 | 363.8499999999999 | 220.7476459510358 |
| all_correct | 251.19978632478632 | 348.75 | 240.44230769230768 |

本次all-wrong平均输出237.97tokens低于all-correct的251.20，但P95为378.00，高于all-correct的348.75。平均长度和尾部并不呈现同一个方向；group label也不等于真实difficulty标签。
...
Formal answer-tag closure=99.975%，think closure=97.78%，truncation=0.00%；strict format=86.925%，fallback=0.00%，ambiguous=10.65%，unparseable=13.08%。正确但格式不合格0条、错误但格式正确1406条。parse errors：{'invalid_thinking_structure': 426, 'unclosed_thinking': 91, 'illegal_or_empty_options': 6}。answer标签闭合不等于严格结构完全合法。

空reasoning4条，<=10tokens的近空reasoning425条，其中合法格式+空reasoning4条。5%的format奖励上界小于correctness0.8，但该项是否影响group内排序由这些真实结构差异决定，不能仅凭SFT源内验证100%格式就说它没有影响。

| acc | sem mean | sem P50 | sem P95 | n |
| --- | ---: | ---: | ---: | ---: |
| 0 | 0.6290213419032752 | 0.807647705078125 | 0.9090504050254822 | 1929 |
| 1 | 0.7213746075006224 | 0.8396930694580078 | 0.914563775062561 | 2071 |

wrong+sem>0.9共 **154** 条，其中可解析错选与不可解析分别为{'parsed_wrong': 142, 'unparseable': 12}；高相似+acc0本身不能直接判定为医学事实错误。补充固定阈值>0.8为1005条。correct+sem<0.5为304条，其中缺解释或空reasoning的0值另由semantic_reason分开：{'encoded': 3400, 'missing_reference': 596, 'empty_reasoning': 4}。有参考且实际编码的分布另保存在supplementary analysis，避免把源缺失解释成编码器判错。

相关系数（常量变量时为null）：{'acc_sem': 0.14723248629663307, 'acc_format': 0.4018578299615338, 'reward_acc': 0.9969800307738761}。total主要由binary correctness隔开，由公式不变量和原始值共同验证；相关性不构成因果推断。高语义错答的gated contribution仍为0；正确final answer中的错误解释仍可能获得shaping，这是明确保留的局限。

![全量profiling图](../../experiments/stage2/figures/rollout_profile.png)

## 8. 人工定性复核、bad cases和失败保留

完成 **80** 条完整trajectory的 **manual qualitative review**，由当前执行代理逐条阅读，不是人类医生审定或clinical validation，也没有调用外部GPT/Doubao等LLM Judge API或外部scorer。先读每个0–4桶前三组共60条，再读全量自动挖出的高sem错误/低sem正确/多选/格式/长度等补充样本。每条记录raw输出SHA、完整阅读范围、解析一致性与独立观察。医学事实无法确定时一律NOT_ASSESSED；不把代理阅读包装成人类医生审定。
...
| 阶段 | NVML peak GiB | 当前进程torch allocated peak GiB | 当前进程torch reserved peak GiB |
| --- | ---: | ---: | ---: |
| vLLM generation | 30.345 | 0.000 | 0.000 |
| BGE semantic scoring | 3.757 | 2.451 | 2.746 |

Formal生成输出 **950213tokens**，本次generation调用总时长 **5213.106s**、吞吐 **182.274 output tokens/s**；冷加载/控制/sleep/wake另列。生成worker计时5429.929s，BGE加载+编码65.965s。共用单张RTX5880 Ada46,068MiB。生成与scoring峰值NVML、allocated/reserved分别在generation_summary和semantic_memory保存，不能把父进程torch的0 allocated解释为vLLM引擎无显存占用。

每个G4 parent输入计数 **135234tokens**，四条trajectory逻辑输入计数 **540936tokens**。前者历史字段名带shared_prefill，但本项目只实测请求token IDs，没有测GPU kernel prefill工作量；prefix cache关闭，不声称节省该倍率的prefill计算。控制额外prompt/output=640/40tokens。失败token=0，retry output=0，未结束请求=[]，真实generation attempts=1。生成批wall共用于该批response记录，并明确不是每条请求的独立latency；不能将这列累加当总时间。

所有Stage2 GPU-owning worker的记录时长合计 **1.6214h**，包含smoke/formal生成与顺序GPU语义评分；CPU数据/semantic diagnostic另计。该口径含worker里的CPU处理/sleep，不是GPU利用率积分，未记录的进程退出尾部不补造。完整成本账本：[compute calibration](../../experiments/stage2/compute_calibration.json)。
...
轨迹级51.775%，范围为本次1000个clean train prompts上的4000次生成。

### Q2 all-wrong/mixed/all-correct

{'all_wrong': 235, 'mixed': 531, 'all_correct': 234}；比例{'all_wrong': 0.235, 'mixed': 0.531, 'all_correct': 0.234}。
...
1/P_mixed≈1.8832；初始固定policy一阶估计，不是实际Dynamic训练倍率。

### Q5 CMExam长度

平均237.55、P95=365.00 output tokens。

### Q6 1024是否足够

本次截断0.00%、answer closure99.98%。作为后续共同response cap候选保留1024；未做2048准确率对照，不能外推全部训练中的长尾。

### Q7 format权重是否无影响

严格格式86.92%，仍有结构差异；5%小于correctness主项，但不能称其在group内完全无影响。

### Q8 semantic是否有用

BGE提供部分实体/主题对齐信息，数字与否定错误仍不敏感；作为gated弱shaping保留实验假设，没有证明提升推理质量。

### Q9 wrong且high-semantic

sem>0.9有154条；门控贡献全0，另保留相对最高错误案例。

### Q10 Stage3 readiness

READY_FOR_STAGE3 = YES：pool/parser/reward/G4/acc分组/cap候选/完整分布及账本已明确，仍以最终verifier PASS为阶段门槛。Stage3没有在本轮执行。
## 11. 验收、交接与限制
...
交接：`experiments/stage2/reward_manifest.json`（hash `cc72be1c2348d20f58bf978d047b9b4a2f200d8c70c39f0d0d98506528bfb905`）、candidate pool manifest、formal config、1000×4 group acc_vector、response cap1024候选与相同Stage1 adapter。Stage3必须以acc对比验证真实refill，不得用total reward/sem std替代，不得带未冻结的新reward直接进入Stage4。
...
我把一个完整医疗SFT checkpoint接成了可核验的在线RL前置流水线：固定15k CMExam train候选池，原生vLLM同policy生成1000题×4次，严格解析最终选项，再按真实标签计算正确性门控的语义和格式奖励。实测accuracy51.775%、mixed53.10%，初始采样放大估计1.8832倍。关键收获是embedding高相似不代表推理正确，且mixed可能来自格式失败；这些原始案例与成本都会保留给后续GSPO对照。
...
在单GPU上继承经过验证的vLLM V1/native sampler/batch invariance，实际做adapter正负对照和sleep/wake校验。50×4 smoke后没有触发长度或温度调整，正式同一固定SFT完成全部4000响应，平均输出237.6tokens，mixed53.10%。据此估算后续Dynamic初始生成约放大1.8832倍，但还没有训练，接受率将随policy变。完整raw、failures、逐组acc_vector、奖励分解和定性review说明了实验何时可信、何时结论必须受限。

## 14. 深挖准备

- **为什么不用LLM Judge？** 选项正确性有固定ground truth，严格集合核对便于重算，外部judge会引入额外成本与不稳定；推理事实无法判断时明确NOT_ASSESSED。
